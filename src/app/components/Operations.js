...
function Operations() {
  ...
  async function getTask() {
    try {
      const Get = await axios.get("/api/list"); // <-- Changed
      setTaskList(Get.data.tasks);
    } catch (error) {
      toast.error("Failed loading tasks");
    }
  ...
  async function updateTask() {
    try {
      const Edit = await axios.patch("/api/edit", { // <-- Changed
        id: EditTask,
        Task: Data.Task,
        Description: Data.Description,
      });
  ...
  async function handleDelete(userid) {
    try {
      const Delete = await axios.delete("/api/delete", { // <-- Changed
        data: { id: userid },
      });
      if (Delete) {
        toast.success("Task deleted successfully");
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (EditTask) {
        ...
        const Send = await axios.post("/api/add", Data); // <-- Changed
        toast.success("Task created");
        getTask();
      }
      setData({ Task: "", Description: "" });
